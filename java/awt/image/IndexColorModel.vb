...
Namespace java.awt.image
	...
	Public Class IndexColorModel
        ...
        '''         than 1 </exception>
        Public Sub New(bits As Integer, size As Integer, r() As Byte, g() As Byte, b() As Byte)
            MyBase.New(bits, opaqueBits, java.awt.Color.colorSpace.getInstance(java.awt.Color.colorSpace.CS_sRGB), False, False, OPAQUE, ColorModel.getDefaultTransferType(bits))
        ...
        '''          1 </exception>
        Public Sub New(bits As Integer, size As Integer, r() As SByte(), g() As SByte(), b() As SByte(), trans As Integer)
            MyBase.New(bits, opaqueBits, java.awt.Color.colorSpace.getInstance(java.awt.Color.colorSpace.CS_sRGB), False, False, OPAQUE, ColorModel.getDefaultTransferType(bits))
            If bits < 1 OrElse bits > 16 Then Throw New IllegalArgumentException("Number of bits must be between" & " 1 and 16.")
            rGBsGBs(size, r, g, b, Nothing)
        ...
        ''' <exception cref="IllegalArgumentException"> if <code>size</code> is less
        '''           than 1 </exception>
        Public Sub New(bits As Integer, size As Integer, r() As SByte, g() As SByte, b() As SByte, a() As SByte)
            MyBase.New(bits, alphaBits, java.awt.Color.colorSpace.getInstance(java.awt.Color.colorSpace.CS_sRGB), True, False, TRANSLUCENT, ColorModel.getDefaultTransferType(bits))
            If bits < 1 OrElse bits > 16 Then Throw New IllegalArgumentException("Number of bits must be between" & " 1 and 16.")
        ...
        ''' <exception cref="IllegalArgumentException"> if <code>size</code> is less
        '''           than 1 </exception>
        Public Sub New(bits As Integer, size As Integer, cmap() As SByte, start As Integer, hasalpha As Boolean)
            Me.New(bits, size, cmap, start, hasalpha, -1)
            If bits < 1 OrElse bits > 16 Then Throw New IllegalArgumentException("Number of bits must be between" & " 1 and 16.")
        End Sub
        ...
        ''' <exception cref="IllegalArgumentException"> if <code>size</code> is less than
        '''               1 </exception>
        Public Sub New(bits As Integer, size As Integer, cmap() As SByte, start As Integer, hasalpha As Boolean, trans As Integer)
        ...
        Public Sub New(bits As Integer, size As Integer, cmap() As Integer, start As Integer, hasalpha As Boolean, trans As Integer, transferType As Integer)
            ' REMIND: This assumes the ordering: RGB[A]
            MyBase.New(bits, opaqueBits, java.awt.Color.colorSpace.getInstance(java.awt.Color.colorSpace.CS_sRGB), False, False, OPAQUE, transferType)
        ...
        ''' 
        ''' @since 1.3 </exception>
        Public Sub New(bits As Integer, size As Integer, cmap() As Integer, start As Integer, transferType As Integer, validBits As System.Numerics.BigInteger)
            MyBase.New(bits, alphaBits, java.awt.Color.colorSpace.getInstance(java.awt.Color.colorSpace.CS_sRGB), True, False, TRANSLUCENT, transferType)
        ...
        Private Sub rGBsGBs(size As Integer, r() As SByte, g() As SByte, b() As SByte, a() As SByte)
            If size < 1 Then Throw New IllegalArgumentException("Map size (" & size & ") must be >= 1")
            map_size = size
        ...
        Private Sub rGBsGBs(size As Integer, cmap() As Integer, start As Integer, hasalpha As Boolean)
            map_size = size
            rgb = New Integer(calcRealMapSize(pixel_bits, size) - 1) {}
            Dim j As Integer = start
            Dim transparency_Renamed As Integer = OPAQUE
        ...
        Private Function calcRealMapSize(bits As Integer, size As Integer) As Integer
            Dim newSize As Integer = System.Math.Max(1 << bits, size)
            Return System.Math.Max(newSize, 256)
        End Function

        Private Function allValid() As System.Numerics.BigInteger
        ...
        Public Function componentSize() As Integer()
        ...
        Public Function mapSize() As Integer
            Return map_size
        End Function
        ...
        Public Function transparentPixel() As Integer
        ...
        Public Sub getGreens(g() As SByte)
            For i As Integer = 0 To map_size - 1
                g(i) = CByte(rgb(i) >> 8)
            Next i
        End Sub
        ...
        Public Sub getBlues(b() As SByte)
            For i As Integer = 0 To map_size - 1
                b(i) = CByte(rgb(i))
            Next i
        End Sub
        ...
        Public Sub getAlphas(a() As SByte)
            For i As Integer = 0 To map_size - 1
                a(i) = CByte(rgb(i) >> 24)
            Next i
        End Sub
        ...
        '''        are copied. </param>
        Public Sub getRGBs(rgb() As Integer)
            Array.Copy(Me.rgb, 0, rgb, 0, map_size)
        End Sub

        Private Sub transparentPixelxel(trans As Integer)
        ...
        Private Sub transparencyncy(transparency As Integer)
        ...
        ''' </summary>
        Private Sub calculatePixelMask()
        ...
        Public Function getRed(pixel As Integer) As Integer
        ...
        Public Function getGreen(pixel As Integer) As Integer
        ...
        Public Function getBlue(pixel As Integer) As Integer
        ...
        Public Function getAlpha(pixel As Integer) As Integer
        ...
        Public Function getRGB(pixel As Integer) As Integer
            Return rgb(pixel And pixel_mask)
        End Function

        Private Const CACHESIZE As Integer = 40
        Private lookupcache() As Integer = New Integer(CACHESIZE - 1) {}
        ...
        ''' <seealso cref= ColorModel#hasAlpha </seealso>
        ''' <seealso cref= ColorModel#getNumComponents </seealso>
        Public Function getComponents(pixel As Object, components As Integer(), offset As Integer) As Integer()
        ...
        ''' <exception cref="UnsupportedOperationException"> if <code>transferType</code>
        '''         is invalid </exception>
        Public Function getDataElement(components As Integer(), offset As Integer) As Integer
        ...
        ''' <seealso cref= WritableRaster#setDataElements </seealso>
        ''' <seealso cref= SampleModel#setDataElements </seealso>
        Public Function getDataElements(components As Integer(), offset As Integer, pixel As Object) As Object
            Dim rgb_Renamed As Integer = (components(offset + 0) << 16) Or (components(offset + 1) << 8) Or (components(offset + 2))
            If supportsAlpha Then
                rgb_Renamed = rgb_Renamed Or (components(offset + 3) << 24)
            Else
        ...
        Public Function createCompatibleWritableRaster(w As Integer, h As Integer) As WritableRaster
        ...
        '''   </returns>
        Public Function isCompatibleRaster(Raster As Raster) As Boolean
        ...
        ''' <seealso cref= SampleModel </seealso>
        Public Function createCompatibleSampleModel(w As Integer, h As Integer) As SampleModel
        ...
        ''' otherwise. </returns>
        ''' <seealso cref= SampleModel </seealso>
        Public Function isCompatibleSampleModel(sm As SampleModel) As Boolean
            ' fix 4238629
        ...
        Public Function convertToIntDiscrete(raster As Raster, forceARGB As Boolean) As BufferedImage
        ...
        ''' @since 1.3 </returns>
        Public Function isValid(pixel As Integer) As Boolean

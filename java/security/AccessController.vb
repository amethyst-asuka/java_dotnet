...
Namespace java.security
	...
	''' 
	''' <p> The <seealso cref="#checkPermission(Permission) checkPermission"/> method
	...
	Public NotInheritable Class AccessController
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
		Public Shared Function doPrivilegedWithCombiner(Of T)(ByVal action As PrivilegedAction(Of T)) As T
			...
			Return AccessController.doPrivileged(action, preserveCombiner(dc, sun.reflect.Reflection.callerClass))
		End Function
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
		...
		Private Shared Function createWrapper(ByVal combiner As DomainCombiner, ByVal caller As [Class], ByVal parent As AccessControlContext, ByVal context As AccessControlContext, ByVal perms As Permission()) As AccessControlContext
			...
			If context IsNot Nothing AndAlso (Not context.authorized) AndAlso System.securityManager IsNot Nothing AndAlso (Not callerPD_Renamed.impliesCreateAccessControlContext()) Then
				Dim nullPD As New ProtectionDomain(Nothing, Nothing)
				Return New AccessControlContext(New ProtectionDomain() { nullPD })
			Else
				Return New AccessControlContext(callerPD_Renamed, combiner, parent, context, perms)
			End If
		End Function
...
'JAVA TO VB CONVERTER TODO TASK: Replace 'unknown' with the appropriate dll name:
		<DllImport("unknown")> _
		Private Shared Function getStackAccessControlContext() As AccessControlContext
		End Function
...
'JAVA TO VB CONVERTER TODO TASK: Replace 'unknown' with the appropriate dll name:
        ...
        Public Shared Sub checkPermission(ByVal perm As Permission)
			...
			Dim stack As AccessControlContext = stackAccessControlContext
			' if context is null, we had privileged system code on the stack.
			If stack Is Nothing Then
				Dim debug As sun.security.util.Debug = AccessControlContext.debug
				...
				If dumpDebug AndAlso sun.security.util.Debug.isOn("domain") Then debug.println("domain (context is null)")

				If dumpDebug Then debug.println("access allowed " & perm)
				Return
			End If

			Dim acc As AccessControlContext = stack.optimize()

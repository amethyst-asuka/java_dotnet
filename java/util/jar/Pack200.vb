...
Namespace java.util.jar
	...
	''' @since 1.5
	''' </summary>
	Public MustInherit Class Pack200
		...
		Public Shared Function newPacker() As Packer
			Return CType(newInstance(PACK_PROVIDER), Packer)
		End Function
		...
		Public Shared Function newUnpacker() As Unpacker
			Return CType(newInstance(UNPACK_PROVIDER), Unpacker)
		End Function
...
'			String DEFLATE_HINT = "pack.deflate.hint";
			...
			''' set the modification time of each element of its output file. </summary>
...
'			String LATEST = "latest";
			...
			''' 
			''' <p>
			''' The returned map implements all optional <seealso cref="SortedMap"/> operations </summary>
			''' <returns> A sorted association of property key strings to property
			''' values. </returns>
			Function properties() As java.util.SortedMap(Of String, String)

			''' <summary>
			''' Takes a JarFile and converts it into a Pack200 archive.
			''' <p>
			''' Closes its input but not its output.  (Pack200 archives are appendable.) </summary>
...
'			String PROGRESS = "unpack.progress";
			...
			''' <p>
			''' Closes its input but not its output.  (The output can accumulate more elements.) </summary>
			...
			''' a JarOutputStream.
			''' <p>
			''' Does not close its output.  (The output can accumulate more elements.) </summary>
		...
		Private Shared Function newInstance(ByVal prop As String) As Object
			...
			Try
				...
				If impl Is Nothing Then
					...
					If implName IsNot Nothing AndAlso (Not implName.Equals("")) Then
						impl = Type.GetType(implName)
					ElseIf PACK_PROVIDER.Equals(prop) Then
						impl = GetType(com.sun.java.util.jar.pack.PackerImpl)
					Else
						impl = GetType(com.sun.java.util.jar.pack.UnpackerImpl)
					End If
				End If
			...
			Catch e As IllegalAccessException
				Throw New [Error]("Cannot access class: " & implName & ":" & vbLf & "check property " & prop & " in your properties file.", e)
			End Try
		End Function

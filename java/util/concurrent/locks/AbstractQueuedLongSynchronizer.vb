...
Namespace java.util.concurrent.locks
	...
	''' 64 bits of state.
	''' 
	''' <p>See <seealso cref="AbstractQueuedSynchronizer"/> for usage
	''' notes and examples.
	''' 
	''' @since 1.6
	''' @author Doug Lea
	''' </summary>
	<Serializable> _
	Public MustInherit Class AbstractQueuedLongSynchronizer
		Inherits AbstractOwnableSynchronizer

		Private Const serialVersionUID As Long = 7373984972572414692L
...
'JAVA TO VB CONVERTER TODO TASK: There is no VB equivalent to 'volatile':
		...
		Protected Friend Property state As Long
			Get
				Return state
			End Get
			Set(ByVal newState As Long)
				state = newState
			End Set
		End Property
		...
		Protected Friend Function compareAndSetState(ByVal expect As Long, ByVal update As Long) As Boolean
			' See below for intrinsics setup to support this
			Return unsafe.compareAndSwapLong(Me, stateOffset, expect, update)
		End Function
	...
	'         
		...
		Private Sub setHeadAndPropagate(ByVal node_Renamed As Node, ByVal propagate As Long)
	...
	'     
		...
		Friend Function acquireQueued(ByVal node_Renamed As Node, ByVal arg As Long) As Boolean
		...
		''' <param name="arg"> the acquire argument </param>
		Private Sub doAcquireInterruptibly(ByVal arg As Long)
		...
		Private Function doAcquireNanos(ByVal arg As Long, ByVal nanosTimeout As Long) As Boolean
		...
		''' <param name="arg"> the acquire argument </param>
		Private Sub doAcquireShared(ByVal arg As Long)
			...
			Try
				...
				Do
					Dim p As Node = node_Renamed.predecessor()
					If p Is head Then
						Dim r As Long = tryAcquireShared(arg)
						If r >= 0 Then
		...
		''' <param name="arg"> the acquire argument </param>
		Private Sub doAcquireSharedInterruptibly(ByVal arg As Long)
			Dim node_Renamed As Node = addWaiter(Node.SHARED)
			Dim failed As Boolean = True
			Try
				Do
		...
		''' <param name="nanosTimeout"> max wait time </param>
		''' <returns> {@code true} if acquired </returns>
		Private Function doAcquireSharedNanos(ByVal arg As Long, ByVal nanosTimeout As Long) As Boolean
			If nanosTimeout <= 0L Then Return False
			Dim deadline As Long = System.nanoTime() + nanosTimeout
			Dim node_Renamed As Node = addWaiter(Node.SHARED)
			Dim failed As Boolean = True
		...
		Protected Friend Overridable Function tryAcquire(ByVal arg As Long) As Boolean
		...
		'''         correctly. </exception>
		''' <exception cref="UnsupportedOperationException"> if exclusive mode is not supported </exception>
		Protected Friend Overridable Function tryRelease(ByVal arg As Long) As Boolean
			Throw New UnsupportedOperationException
		End Function
		...
		Protected Friend Overridable Function tryAcquireShared(ByVal arg As Long) As Long
			Throw New UnsupportedOperationException
		End Function
		...
		'''         correctly. </exception>
		''' <exception cref="UnsupportedOperationException"> if shared mode is not supported </exception>
		Protected Friend Overridable Function tryReleaseShared(ByVal arg As Long) As Boolean
			Throw New UnsupportedOperationException
		End Function
		...
		Public Sub acquire(ByVal arg As Long)
		...
		Public Sub acquireInterruptibly(ByVal arg As Long)
			If Thread.interrupted() Then Throw New InterruptedException
		...
		''' <exception cref="InterruptedException"> if the current thread is interrupted </exception>
		Public Function tryAcquireNanos(ByVal arg As Long, ByVal nanosTimeout As Long) As Boolean
			If Thread.interrupted() Then Throw New InterruptedException
		...
		Public Function release(ByVal arg As Long) As Boolean
		...
		'''        and can represent anything you like. </param>
		Public Sub acquireShared(ByVal arg As Long)
		...
		''' you like. </param>
		''' <exception cref="InterruptedException"> if the current thread is interrupted </exception>
		Public Sub acquireSharedInterruptibly(ByVal arg As Long)
			If Thread.interrupted() Then Throw New InterruptedException
		...
		''' <returns> {@code true} if acquired; {@code false} if timed out </returns>
		''' <exception cref="InterruptedException"> if the current thread is interrupted </exception>
		Public Function tryAcquireSharedNanos(ByVal arg As Long, ByVal nanosTimeout As Long) As Boolean
			If Thread.interrupted() Then Throw New InterruptedException
		...
		Public Function releaseShared(ByVal arg As Long) As Boolean
...
'JAVA TO VB CONVERTER TODO TASK: Assignments within expressions are not supported in VB
	...
	'         
		...
		Friend Function fullyRelease(ByVal node_Renamed As Node) As Long
			Dim failed As Boolean = True
			Try
				Dim savedState As Long = state
		...
		Public Class ConditionObject
			...
			Private ReadOnly outerInstance As AbstractQueuedLongSynchronizer
			...
			''' </summary>
			Public Sub New(ByVal outerInstance As AbstractQueuedLongSynchronizer)
			...
			Public Sub awaitUninterruptibly() Implements Condition.awaitUninterruptibly
				...
				Dim savedState As Long = outerInstance.fullyRelease(node_Renamed)
				Dim interrupted As Boolean = False
	...
	'         
			...
			''' </summary>
			''' <returns> {@code true} if owned </returns>
			Friend Function isOwnedBy(ByVal sync As AbstractQueuedLongSynchronizer) As Boolean
				Return sync Is AbstractQueuedLongSynchronizer.this
			End Function
		...
		Shared Sub New()
			Try
				stateOffset = unsafe.objectFieldOffset(GetType(AbstractQueuedLongSynchronizer).getDeclaredField("state"))
				headOffset = unsafe.objectFieldOffset(GetType(AbstractQueuedLongSynchronizer).getDeclaredField("head"))
				tailOffset = unsafe.objectFieldOffset(GetType(AbstractQueuedLongSynchronizer).getDeclaredField("tail"))

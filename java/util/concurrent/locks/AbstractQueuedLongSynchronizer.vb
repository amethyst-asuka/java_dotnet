...
'JAVA TO VB CONVERTER TODO TASK: There is no VB equivalent to 'volatile':
		...
		Protected Friend Function compareAndSetState(  expect As Long,   update As Long) As Boolean
	...
	'         
		...
		Private Sub setHeadAndPropagate(  node_Renamed As Node,   propagate As Long)
	...
	'     
		...
		Friend Function acquireQueued(  node_Renamed As Node,   arg As Long) As Boolean
		...
		''' <param name="arg"> the acquire argument </param>
		Private Sub doAcquireInterruptibly(  arg As Long)
		...
		Private Function doAcquireNanos(  arg As Long,   nanosTimeout As Long) As Boolean
		...
		''' <param name="arg"> the acquire argument </param>
		Private Sub doAcquireShared(  arg As Long)
		...
		''' <param name="arg"> the acquire argument </param>
		Private Sub doAcquireSharedInterruptibly(  arg As Long)
			Dim node_Renamed As Node = addWaiter(Node.SHARED)
			Dim failed As Boolean = True
			Try
				Do
		...
		''' <param name="nanosTimeout"> max wait time </param>
		''' <returns> {@code true} if acquired </returns>
		Private Function doAcquireSharedNanos(  arg As Long,   nanosTimeout As Long) As Boolean
			If nanosTimeout <= 0L Then Return False
			Dim deadline As Long = System.nanoTime() + nanosTimeout
			Dim node_Renamed As Node = addWaiter(Node.SHARED)
			Dim failed As Boolean = True
		...
		Protected Friend Overridable Function tryAcquire(  arg As Long) As Boolean
		...
		'''         correctly. </exception>
		''' <exception cref="UnsupportedOperationException"> if exclusive mode is not supported </exception>
		Protected Friend Overridable Function tryRelease(  arg As Long) As Boolean
			Throw New UnsupportedOperationException
		End Function
		...
		Protected Friend Overridable Function tryAcquireShared(  arg As Long) As Long
			Throw New UnsupportedOperationException
		End Function
		...
		'''         correctly. </exception>
		''' <exception cref="UnsupportedOperationException"> if shared mode is not supported </exception>
		Protected Friend Overridable Function tryReleaseShared(  arg As Long) As Boolean
			Throw New UnsupportedOperationException
		End Function
		...
		Public Sub acquire(  arg As Long)
		...
		Public Sub acquireInterruptibly(  arg As Long)
			If Thread.interrupted() Then Throw New InterruptedException
		...
		''' <exception cref="InterruptedException"> if the current thread is interrupted </exception>
		Public Function tryAcquireNanos(  arg As Long,   nanosTimeout As Long) As Boolean
			If Thread.interrupted() Then Throw New InterruptedException
		...
		Public Function release(  arg As Long) As Boolean
		...
		'''        and can represent anything you like. </param>
		Public Sub acquireShared(  arg As Long)
		...
		''' you like. </param>
		''' <exception cref="InterruptedException"> if the current thread is interrupted </exception>
		Public Sub acquireSharedInterruptibly(  arg As Long)
			If Thread.interrupted() Then Throw New InterruptedException
		...
		''' <returns> {@code true} if acquired; {@code false} if timed out </returns>
		''' <exception cref="InterruptedException"> if the current thread is interrupted </exception>
		Public Function tryAcquireSharedNanos(  arg As Long,   nanosTimeout As Long) As Boolean
			If Thread.interrupted() Then Throw New InterruptedException
		...
		Public Function releaseShared(  arg As Long) As Boolean
...
'JAVA TO VB CONVERTER TODO TASK: Assignments within expressions are not supported in VB
	...
	'         
		...
		Friend Function fullyRelease(  node_Renamed As Node) As Long
			Dim failed As Boolean = True
			Try
		...
		Public Class ConditionObject
			...
			''' </summary>
			Public Sub New(  outerInstance As AbstractQueuedLongSynchronizer)
	...
	'         
			...
			''' </summary>
			''' <returns> {@code true} if owned </returns>
			Friend Function isOwnedBy(  sync As AbstractQueuedLongSynchronizer) As Boolean

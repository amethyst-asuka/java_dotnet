...
Namespace java.util.concurrent
	...
	''' 
	''' <p>Commands submitted using the <seealso cref="Executor#execute(Runnable)"/>
	...
	''' 
	''' <p>The <seealso cref="Executors"/> class provides convenient factory methods for
	...
	''' 
	'''   public  Sub  beepForAnHour() {
	'''     final Runnable beeper = new Runnable() {
	'''       public  Sub  run() { System.out.println("beep"); }
	'''     };
	'''     final ScheduledFuture<?> beeperHandle =
	'''       scheduler.scheduleAtFixedRate(beeper, 10, 10, SECONDS);
	'''     scheduler.schedule(new Runnable() {
	'''       public  Sub  run() { beeperHandle.cancel(true); }
	'''     }, 60 * 60, SECONDS);
	'''   }
	''' }}</pre>
	''' 

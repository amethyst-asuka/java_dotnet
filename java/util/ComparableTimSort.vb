...
Namespace java.util
	...
	''' @author Josh Bloch
	''' </summary>
	Friend Class ComparableTimSort
		''' <summary>
	...
	'         *  java.lang.[Integer].MAX_VALUE-4, if array is filled by the worst case stack size
	...
	'     
		...
		Friend Shared Sub sort(ByVal a As Object(), ByVal lo As Integer, ByVal hi As Integer, ByVal work As Object(), ByVal workBase As Integer, ByVal workLen As Integer)
			...
			If nRemaining < MIN_MERGE Then
				Dim initRunLen As Integer = countRunAndMakeAscending(a, lo, hi)
			...
			''' </summary>
			Dim ts As New ComparableTimSort(a, work, workBase, workLen)
			Dim minRun As Integer = minRunLength(nRemaining)
			Do
				' Identify next run
				Dim runLen As Integer = countRunAndMakeAscending(a, lo, hi)
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
		Private Shared Sub binarySort(ByVal a As Object(), ByVal lo As Integer, ByVal hi As Integer, ByVal start As Integer)
			...
			Do While start < hi
				Dim pivot As Comparable = CType(a(start), Comparable)
	...
	'             
				Do While left < right
					...
					If pivot.CompareTo(a(mid)) < 0 Then
						right = mid
					Else
						left = mid + 1
					End If
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
		Private Shared Function countRunAndMakeAscending(ByVal a As Object(), ByVal lo As Integer, ByVal hi As Integer) As Integer
			...
			Dim tempVar As Boolean = CType(a(runHi), Comparable).CompareTo(a(lo)) < 0
			runHi += 1
			If tempVar Then ' Descending
				Do While runHi < hi AndAlso CType(a(runHi), Comparable).CompareTo(a(runHi - 1)) < 0
					runHi += 1
				Loop
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
	...
	'         
			Dim k As Integer = gallopRight(CType(a(base2), Comparable(Of Object)), a, base1, len1, 0)
			Debug.Assert(k >= 0)
			base1 += k
			len1 -= k
	...
	'         
			len2 = gallopLeft(CType(a(base1 + len1 - 1), Comparable(Of Object)), a, base2, len2, len2 - 1)
			Debug.Assert(len2 >= 0)
		...
		Private Shared Function gallopLeft(ByVal key As Comparable(Of Object), ByVal a As Object(), ByVal base As Integer, ByVal len As Integer, ByVal hint As Integer) As Integer
			...
			If key.CompareTo(a(base + hint)) > 0 Then
				' Gallop right until a[base+hint+lastOfs] < key <= a[base+hint+ofs]
				Dim maxOfs As Integer = len - hint
				Do While ofs < maxOfs AndAlso key.CompareTo(a(base + hint + ofs)) > 0
					lastOfs = ofs
					ofs = (ofs << 1) + 1
	...
	'         
			...
			Do While lastOfs < ofs
				...
				If key.CompareTo(a(base + m)) > 0 Then
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
		Private Sub mergeLo(ByVal base1 As Integer, ByVal len1 As Integer, ByVal base2 As Integer, ByVal len2 As Integer)
			...
			Dim a As Object() = Me.a ' For performance
			Dim tmp As Object() = ensureCapacity(len1)
			...
			Dim minGallop As Integer = Me.minGallop ' Use local variable for performance
		outer:
			Do
	...
	'             
				Do
					...
					If CType(a(cursor2), Comparable).CompareTo(tmp(cursor1)) < 0 Then
						a(dest) = a(cursor2)
						cursor2 += 1
						dest += 1
						count2 += 1
	...
	'             
				Do
					Debug.Assert(len1 > 1 AndAlso len2 > 0)
					count1 = gallopRight(CType(a(cursor2), Comparable), tmp, cursor1, len1, 0)
					If count1 <> 0 Then
					...
					count2 = gallopLeft(CType(tmp(cursor1), Comparable), a, cursor2, len2, 0)
					If count2 <> 0 Then
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
		Private Sub mergeHi(ByVal base1 As Integer, ByVal len1 As Integer, ByVal base2 As Integer, ByVal len2 As Integer)
			...
			Dim a As Object() = Me.a ' For performance
			Dim tmp As Object() = ensureCapacity(len2)
	...
	'             
				Do
					...
					If CType(tmp(cursor2), Comparable).CompareTo(a(cursor1)) < 0 Then
						a(dest) = a(cursor1)
						cursor1 -= 1
						dest -= 1
						count1 += 1
	...
	'             
				Do
					Debug.Assert(len1 > 0 AndAlso len2 > 1)
					count1 = len1 - gallopRight(CType(tmp(cursor2), Comparable), a, base1, len1, len1 - 1)
					If count1 <> 0 Then
						dest -= count1
						cursor1 -= count1
						len1 -= count1
					...
					count2 = len2 - gallopLeft(CType(a(cursor1), Comparable), tmp, tmpBase, len2, len2 - 1)
					If count2 <> 0 Then
						dest -= count2
						cursor2 -= count2
						len2 -= count2
						Array.Copy(tmp, cursor2 + 1, a, dest + 1, count2)
						If len2 <= 1 Then GoTo outer ' len2 == 1 || len2 == 0
					End If
					a(dest) = a(cursor1)
					cursor1 -= 1
					dest -= 1
		...
		Private Function ensureCapacity(ByVal minCapacity As Integer) As Object()
			If tmpLen < minCapacity Then
				...
				If newSize < 0 Then ' Not bloody likely!
					newSize = minCapacity
				Else
					newSize = System.Math.Min(newSize, CInt(CUInt(a.Length) >> 1))
				End If

'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
				Dim newArray As Object() = New Object(newSize - 1){}
				tmp = newArray
				tmpLen = newSize
				tmpBase = 0
			End If

...
Namespace java.util
	...
	Public MustInherit Class AbstractSet(Of E)
...
'JAVA TO VB CONVERTER TODO TASK: Java wildcard generics are not converted to .NET:
			Dim c As Collection(Of ?) = CType(o, Collection(Of ?))
		...
		Public Overrides Function GetHashCode() As Integer
			Dim h As Integer = 0
			Dim i As [Iterator](Of E) = [iterator]()
			Do While i.MoveNext()
				Dim obj As E = i.Current
				If obj IsNot Nothing Then h += obj.GetHashCode()
			Loop
			Return h
		End Function
		...
		Public Overridable Function removeAll(Of T1)(ByVal c As Collection(Of T1)) As Boolean Implements Set(Of E).removeAll
			...
			If size() > c.size() Then
				Dim i As [Iterator](Of ?) = c.GetEnumerator()
'JAVA TO VB CONVERTER TODO TASK: Java wildcard generics are not converted to .NET:
				Do While i.hasNext()

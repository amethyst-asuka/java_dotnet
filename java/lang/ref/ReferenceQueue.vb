...
Namespace java.lang.ref
	...
	Public Class ReferenceQueue(Of T)
		...
		Private Class Lock
		End Class
		Private lock As New Lock
'JAVA TO VB CONVERTER TODO TASK: There is no VB equivalent to 'volatile':
'JAVA TO VB CONVERTER TODO TASK: Java wildcard generics are not converted to .NET:
		Private head As Reference(Of ? As T) = Nothing
		Private queueLength As Long = 0

		Friend Overridable Function enqueue(Of T1 As T)(ByVal r As Reference(Of T1)) As Boolean ' Called only by Reference class
			SyncLock lock
				' Check that since getting the lock this reference hasn't already been
				' enqueued (and even then removed)
'JAVA TO VB CONVERTER TODO TASK: Java wildcard generics are not converted to .NET:
				Dim queue As ReferenceQueue(Of ?) = r.queue
...
'JAVA TO VB CONVERTER TODO TASK: Java wildcard generics are not converted to .NET:
		Private Function reallyPoll() As Reference(Of ? As T) ' Must hold lock
'JAVA TO VB CONVERTER TODO TASK: Java wildcard generics are not converted to .NET:
			Dim r As Reference(Of ? As T) = head
...
'JAVA TO VB CONVERTER TODO TASK: Java wildcard generics are not converted to .NET:
		Public Overridable Function poll() As Reference(Of ? As T)
...
'JAVA TO VB CONVERTER TODO TASK: Java wildcard generics are not converted to .NET:
		Public Overridable Function remove(ByVal timeout As Long) As Reference(Of ? As T)
			If timeout < 0 Then Throw New IllegalArgumentException("Negative timeout value")
			SyncLock lock
'JAVA TO VB CONVERTER TODO TASK: Java wildcard generics are not converted to .NET:
				Dim r As Reference(Of ? As T) = reallyPoll()
...
'JAVA TO VB CONVERTER TODO TASK: Java wildcard generics are not converted to .NET:
		Public Overridable Function remove() As Reference(Of ? As T)
			Return remove(0)
		End Function

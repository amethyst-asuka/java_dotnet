...
Namespace java.lang.invoke
	...
	''' <seealso cref="#asType conversion"/>,
	''' <seealso cref="#bindTo insertion"/>,
	''' <seealso cref="java.lang.invoke.MethodHandles#dropArguments deletion"/>,
	''' and <seealso cref="java.lang.invoke.MethodHandles#filterArguments substitution"/>.
	''' 
	''' <h1>Method handle contents</h1>
	''' Method handles are dynamically and strongly typed according to their parameter and return types.
	''' They are not distinguished by the name or the defining class of their underlying methods.
	''' A method handle must be invoked using a symbolic type descriptor which matches
	''' the method handle's own <seealso cref="#type type descriptor"/>.
	''' <p>
	''' Every method handle reports its type descriptor via the <seealso cref="#type type"/> accessor.
	''' This type descriptor is a <seealso cref="java.lang.invoke.MethodType MethodType"/> object,
	...
	''' called <seealso cref="#invokeExact invokeExact"/> and <seealso cref="#invoke invoke"/>.
	...
	''' must be a valid argument to the receiver's <seealso cref="#asType asType"/> method.
	...
	''' as if by a call to <seealso cref="#asType asType"/>,
	...
	''' But if a match fails, the JVM will throw a <seealso cref="WrongMethodTypeException"/>,
	...
	''' from <seealso cref="java.lang.invoke.MethodHandles.Lookup#findStatic Lookup.findStatic"/>.
	''' There are also conversion methods from Core Reflection API objects,
	''' such as <seealso cref="java.lang.invoke.MethodHandles.Lookup#unreflect Lookup.unreflect"/>.
	''' <p>
	...
	''' the help of <seealso cref="#asVarargsCollector asVarargsCollector"/>.
	''' <p>
	...
	''' be a method which calls <seealso cref="java.util.Objects#equals(Object,Object) Objects.equals"/>
	...
	''' Using factory methods in the <seealso cref="java.lang.invoke.MethodHandles.Lookup Lookup"/> API,
	''' any class member represented by a Core Reflection API object
	''' can be converted to a behaviorally equivalent method handle.
	''' For example, a reflective <seealso cref="java.lang.reflect.Method Method"/> can
	''' be converted to a method handle using
	''' <seealso cref="java.lang.invoke.MethodHandles.Lookup#unreflect Lookup.unreflect"/>.
	...
	''' For example, <seealso cref="java.lang.reflect.Method#getModifiers Method.getModifiers"/>
	''' will report exactly those modifier bits required for any similarly
	''' declared method, including in this case {@code native} and {@code varargs} bits.
	''' <p>
	''' As with any reflected method, these methods (when reflected) may be
	''' invoked via <seealso cref="java.lang.reflect.Method#invoke java.lang.reflect.Method.invoke"/>.
	...
	''' use <seealso cref="java.lang.invoke.MethodHandles#exactInvoker MethodHandles.exactInvoker"/>,
	''' or <seealso cref="java.lang.invoke.MethodHandles#invoker MethodHandles.invoker"/>.
	''' The <seealso cref="java.lang.invoke.MethodHandles.Lookup#findVirtual Lookup.findVirtual"/>
	...
	''' Attempts to create method handles with impossible method types lead to an <seealso cref="IllegalArgumentException"/>.
	...
	''' <seealso cref= MethodHandles
	''' @author John Rose, JSR 292 EG </seealso>
	Public MustInherit Class MethodHandle
		Shared Sub New()
			MethodHandleImpl.initStatics()
			Try
				FORM_OFFSET = UNSAFE.objectFieldOffset(GetType(MethodHandle).getDeclaredField("form"))
		...
		Friend Class PolymorphicSignature
		...
		Private ReadOnly type_Renamed As MethodType
		'private
	 Friend ReadOnly form As LambdaForm
		' form is not private so that invokers can easily fetch it
		'private
	 Friend asTypeCache As MethodHandle
		' asTypeCache is not private so that invokers can easily fetch it
		'non-public
	 Friend customizationCount As SByte
		' customizationCount should be accessible from invokers

		''' <summary>
		''' Reports the type of this method handle.
		''' Every invocation of this method handle via {@code invokeExact} must exactly match this type. </summary>
	 ...
	 Friend Sub New(ByVal type As MethodType, ByVal form As LambdaForm)
			...
			Me.form = form.uncustomize()

			Me.form.prepare() ' TO DO:  Try to delay this step until just before invocation.
	 End Sub

		''' <summary>
		''' Invokes the method handle, allowing any caller type descriptor, but requiring an exact type match.
		''' The symbolic type descriptor at the call site of {@code invokeExact} must
		''' exactly match this method handle's <seealso cref="#type type"/>.
		''' No conversions are allowed on arguments or return values.
		''' <p>
		''' When this method is observed via the Core Reflection API,
		''' it will appear as a single native method, taking an object array and returning an object.
		''' If this native method is invoked directly via
		''' <seealso cref="java.lang.reflect.Method#invoke java.lang.reflect.Method.invoke"/>, via JNI,
		''' or indirectly via <seealso cref="java.lang.invoke.MethodHandles.Lookup#unreflect Lookup.unreflect"/>,
		''' it will throw an {@code UnsupportedOperationException}. </summary>
		''' <param name="args"> the signature-polymorphic parameter list, statically represented using varargs </param>
		''' <returns> the signature-polymorphic result, statically represented using {@code Object} </returns>
		''' <exception cref="WrongMethodTypeException"> if the target's type is not identical with the caller's symbolic type descriptor </exception>
...
'JAVA TO VB CONVERTER TODO TASK: Replace 'unknown' with the appropriate dll name:
		...
		''' <p>
		''' If the call site's symbolic type descriptor exactly matches this method handle's <seealso cref="#type type"/>,
		''' the call proceeds as if by <seealso cref="#invokeExact invokeExact"/>.
		''' <p>
		''' Otherwise, the call proceeds as if this method handle were first
		''' adjusted by calling <seealso cref="#asType asType"/> to adjust this method handle
		''' to the required type, and then the call proceeds as if by
		''' <seealso cref="#invokeExact invokeExact"/> on the adjusted method handle.
		''' <p>
		...
		''' if the callee is not a <seealso cref="#asVarargsCollector variable arity collector"/>.
		''' <p>
		''' When this method is observed via the Core Reflection API,
		''' it will appear as a single native method, taking an object array and returning an object.
		''' If this native method is invoked directly via
		...
		''' <param name="args"> the signature-polymorphic parameter list, statically represented using varargs </param>
		''' <returns> the signature-polymorphic result, statically represented using {@code Object} </returns>
		''' <exception cref="WrongMethodTypeException"> if the target's type cannot be adjusted to the caller's symbolic type descriptor </exception>
		''' <exception cref="ClassCastException"> if the target's type can be adjusted to the caller, but a reference cast fails </exception>
		''' <exception cref="Throwable"> anything thrown by the underlying method propagates unchanged through the method handle call </exception>
'JAVA TO VB CONVERTER TODO TASK: Replace 'unknown' with the appropriate dll name:
		<DllImport("unknown")> _
		...
		''' of the callee's return type. </summary>
		''' <param name="args"> the signature-polymorphic parameter list, statically represented using varargs </param>
		''' <returns> the signature-polymorphic result, statically represented using {@code Object} </returns>
		'non-public
'JAVA TO VB CONVERTER TODO TASK: Replace 'unknown' with the appropriate dll name:
	 ...
	 End Function
		...
		''' The trailing (not leading) argument must be a MemberName. </summary>
		''' <param name="args"> the signature-polymorphic parameter list, statically represented using varargs </param>
		''' <returns> the signature-polymorphic result, statically represented using {@code Object} </returns>
		'non-public
'JAVA TO VB CONVERTER TODO TASK: Replace 'unknown' with the appropriate dll name:
...
'JAVA TO VB CONVERTER TODO TASK: Replace 'unknown' with the appropriate dll name:
	 ...
	 End Function
		...
		''' to the method handle, as if via an inexact <seealso cref="#invoke invoke"/> from a call site
		...
		''' <param name="arguments"> the arguments to pass to the target </param>
		''' <returns> the result returned by the target </returns>
		''' <exception cref="NullPointerException"> if {@code arguments} is a null reference </exception>
		''' <exception cref="ClassCastException"> if an argument cannot be converted by reference casting </exception>
		''' <exception cref="WrongMethodTypeException"> if the target's type cannot be adjusted to take the given number of {@code Object} arguments </exception>
		''' <exception cref="Throwable"> anything thrown by the target method invocation </exception>
		...
		''' <seealso cref="#invokeExact invokeExact"/> and plain, inexact <seealso cref="#invoke invoke"/>.
		''' The two methods
		''' perform the same steps when the caller's type descriptor exactly m atches
		''' the callee's, but when the types differ, plain <seealso cref="#invoke invoke"/>
		...
		''' <seealso cref="#asVarargsCollector described elsewhere"/>.
		...
		''' causing a <seealso cref="java.lang.NullPointerException NullPointerException"/>.
		''' An unboxing operation or a reference cast may also fail on a reference
		''' to an object of the wrong type,
		''' causing a <seealso cref="java.lang.ClassCastException ClassCastException"/>.
		...
		''' <returns> a method handle which delegates to {@code this} after performing
		...
		Public Overridable Function asType(ByVal newType As MethodType) As MethodHandle
			...
			Dim atc As MethodHandle = asTypeCached(newType)
			If atc IsNot Nothing Then Return atc
	 ...
	 End Function
		...
		''' <param name="arrayType"> usually {@code Object[]}, the type of the array argument from which to extract the spread arguments </param>
		...
		''' <exception cref="NullPointerException"> if {@code arrayType} is a null reference </exception>
		''' <exception cref="IllegalArgumentException"> if {@code arrayType} is not an array type,
		...
		Public Overridable Function asSpreader(ByVal arrayType As [Class], ByVal arrayLength As Integer) As MethodHandle
			...
			Dim afterSpread As MethodHandle = Me.asType(postSpreadType)
			Dim mh As BoundMethodHandle = afterSpread.rebind()
			Dim lform As LambdaForm = mh.editor().spreadArgumentsForm(1 + spreadArgPos, arrayType, arrayLength)
			Dim preSpreadType As MethodType = postSpreadType.replaceParameterTypes(spreadArgPos, arity, arrayType)
		...
		Private Function asSpreaderChecks(ByVal arrayType As [Class], ByVal arrayLength As Integer) As MethodType
			...
			For i As Integer = nargs - arrayLength To nargs - 1
				...
				End If
			Next i
			If match Then Return mtype
			Dim needType As MethodType = mtype.asSpreaderType(arrayType, arrayLength)
			If Not fail Then Return needType
			' elicit an error:
			Me.asType(needType)
		...
		Private Sub spreadArrayChecks(ByVal arrayType As [Class], ByVal arrayLength As Integer)
			...
			If (arrayLength And &H7F) <> arrayLength Then
				...
				If arrayElement Is GetType(Long) OrElse arrayElement Is GetType(Double) Then Throw newIllegalArgumentException("array length is not legal for long[] or double[]", arrayLength)
			End If
		End Sub
		...
		''' number of collected arguments, use <seealso cref="#asVarargsCollector asVarargsCollector"/> instead.
		''' <p>
		...
		''' }</pre></blockquote> </summary>
		''' <param name="arrayType"> often {@code Object[]}, the type of the array argument which will collect the arguments </param>
		...
		Public Overridable Function asCollector(ByVal arrayType As [Class], ByVal arrayLength As Integer) As MethodHandle
			...
			Dim lform As LambdaForm = mh.editor().collectArgumentArrayForm(1 + collectArgPos, newArray)
			If lform IsNot Nothing Then Return mh.copyWith(resultType, lform)
	 ...
	 Friend Overridable Function asCollectorChecks(ByVal arrayType As [Class], ByVal arrayLength As Integer) As Boolean
			...
			End If
			Throw newIllegalArgumentException("array type not assignable to trailing argument", Me, arrayType)
	 End Function
		...
		''' <p>
		''' When called with <seealso cref="#invokeExact invokeExact"/>, the adapter invokes
		''' the target with no argument changes.
		''' (<em>Note:</em> This behavior is different from a
		''' <seealso cref="#asCollector fixed arity collector"/>,
		''' since it accepts a whole array of indeterminate length,
		''' rather than a fixed number of arguments.)
		''' <p>
		''' When called with plain, inexact <seealso cref="#invoke invoke"/>, if the caller
		...
		''' as if by <seealso cref="#asType asType"/> on a fixed arity
		''' method handle.
		''' <p>
		...
		''' temporarily adapted with a <seealso cref="#asCollector fixed arity collector"/>
		...
		''' <p>
		''' The behavior of <seealso cref="#asType asType"/> is also specialized for
		...
		''' use <seealso cref="#asCollector asCollector"/> instead.
		''' <p>
		...
		''' <exception cref="NullPointerException"> if {@code arrayType} is a null reference </exception>
		''' <exception cref="IllegalArgumentException"> if {@code arrayType} is not an array type
		'''         or {@code arrayType} is not assignable to this method handle's trailing parameter type </exception>
		''' <seealso cref= #asCollector </seealso>
		...
		Public Overridable Function asVarargsCollector(ByVal arrayType As [Class]) As MethodHandle
			...
			Return MethodHandleImpl.makeVarargsCollector(Me, arrayType)
		End Function

		''' <summary>
		''' Determines if this method handle
		''' supports <seealso cref="#asVarargsCollector variable arity"/> calls.
		''' Such method handles arise from the following sources:
		''' <ul>
		''' <li>a call to <seealso cref="#asVarargsCollector asVarargsCollector"/>
		''' <li>a call to a <seealso cref="java.lang.invoke.MethodHandles.Lookup lookup method"/>
		'''     which resolves to a variable arity Java method or constructor
		''' <li>an {@code ldc} instruction of a {@code CONSTANT_MethodHandle}
		'''     which resolves to a variable arity Java method or constructor
		''' </ul> </summary>
		''' <returns> true if this method handle accepts more than one arity of plain, inexact {@code invoke} calls </returns>
		''' <seealso cref= #asVarargsCollector </seealso>
		''' <seealso cref= #asFixedArity </seealso>
		Public Overridable Property varargsCollector As Boolean
			Get
				Return False
			End Get
		End Property
		...
		''' <seealso cref="#asVarargsCollector variable arity"/>,
		...
		''' except that <seealso cref="#isVarargsCollector isVarargsCollector"/>
		''' will be false.
		...
		''' <seealso cref= #asVarargsCollector </seealso>
		''' <seealso cref= #isVarargsCollector </seealso>
		Public Overridable Function asFixedArity() As MethodHandle
			assert((Not varargsCollector))
			Return Me
		End Function
		...
		''' retains its original type and behavior.) </summary>
		...
		''' <exception cref="IllegalArgumentException"> if the target does not have a
		'''         leading parameter type that is a reference type </exception>
		...
		''' </summary>
		''' <returns> a string representation of the method handle </returns>
		Public Overrides Function ToString() As String
			If DEBUG_METHOD_HANDLE_NAMES Then Return "MethodHandle" & debugString()
			Return standardString()
		End Function
		Friend Overridable Function standardString() As String
			Return "MethodHandle" & type_Renamed
		End Function
		''' <summary>
		...
		''' </summary>
		Friend Overridable Function debugString() As String
			Return type_Renamed & " : " & internalForm()+internalProperties()
		End Function
		...
		Friend Overridable Function viewAsType(ByVal newType As MethodType, ByVal [strict] As Boolean) As MethodHandle
			...
			Debug.Assert(viewAsTypeChecks(newType, [strict]))
			Dim mh As BoundMethodHandle = rebind()
			assert(Not(TypeOf CType(mh, MethodHandle) Is DirectMethodHandle))
		...
		Friend Overridable Function viewAsTypeChecks(ByVal newType As MethodType, ByVal [strict] As Boolean) As Boolean
			...
			Else
				assert(type().basicType().isViewableAs(newType.basicType(), True)) : Me, newType
			End If
			Return True
		End Function
		...
		'non-public
		Friend Overridable Function internalForm() As LambdaForm
			Return form
		End Function

		'non-public
		Friend Overridable Function internalMemberName() As MemberName
			Return Nothing ' DMH returns DMH.member
		End Function
		...
		'non-public
		Friend Overridable Function intrinsicName() As MethodHandleImpl.Intrinsic
		...
		Friend Overridable Function withInternalMemberName(ByVal member As MemberName, ByVal isInvokeSpecial As Boolean) As MethodHandle
			...
			Else
				...
				Dim result As MethodHandle = rebind()
				assert(result.internalMemberName() Is Nothing)
				Return result
			End If
		End Function

		'non-public
		Friend Overridable Property invokeSpecial As Boolean
			Get
		...
		'non-public
		Friend Overridable Function internalValues() As Object
			Return Nothing
		End Function

		'non-public
		Friend Overridable Function internalProperties() As Object
		...
		'''  Many transforms are implemented only for BMHs. </summary>
		'''  <returns> a behaviorally equivalent BMH </returns>
		Friend MustOverride Function rebind() As BoundMethodHandle
		...
		Friend Overridable Sub updateForm(ByVal newForm As LambdaForm)
			...
			UNSAFE.putObject(Me, FORM_OFFSET, newForm)
			UNSAFE.fullFence()
		End Sub

		''' <summary>
		''' Craft a LambdaForm customized for this particular MethodHandle </summary>
		'non-public
		Friend Overridable Sub customize()
			If form.customized Is Nothing Then
				Dim newForm As LambdaForm = form.customize(Me)
				updateForm(newForm)
			Else
				assert(form.customized Is Me)
			End If
		End Sub

		Private Shared ReadOnly FORM_OFFSET As Long
	End Class

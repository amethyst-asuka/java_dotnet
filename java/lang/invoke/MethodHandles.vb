...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
		...
		Public NotInheritable Class Lookup
			...
			Private Shared Sub checkUnprivilegedlookupClass(ByVal lookupClass As Class, ByVal allowedModes As Integer)
				...
				If name.StartsWith("java.lang.invoke.") Then Throw newIllegalArgumentException("illegal lookupClass: " & lookupClass)

				' For caller-sensitive MethodHandles.lookup()
				' disallow lookup more restricted packages
				If allowedModes = ALL_MODES AndAlso lookupClass.classLoader Is Nothing Then
					If name.StartsWith("java.") OrElse (name.StartsWith("sun.") AndAlso (Not name.StartsWith("sun.invoke."))) Then Throw newIllegalArgumentException("illegal lookupClass: " & lookupClass)
				End If
			End Sub
...
'JAVA TO VB CONVERTER TODO TASK: Assignments within expressions are not supported in VB
		...
		Private Shared Function permuteArgumentChecks(ByVal reorder As Integer(), ByVal newType As MethodType, ByVal oldType As MethodType) As Boolean
			If newType.returnType() IsNot oldType.returnType() Then Throw newIllegalArgumentException("return types do not match", oldType, newType)
			If reorder.Length = oldType.parameterCount() Then
				...
				For j As Integer = 0 To reorder.Length - 1
					...
					If src IsNot dst Then Throw newIllegalArgumentException("parameter types do not match after reorder", oldType, newType)
				Next j
				If Not bad Then Return True
			End If
			Throw newIllegalArgumentException("bad reorder array: " & java.util.Arrays.ToString(reorder))
		End Function
		...
		Public Shared Function constant(ByVal type As Class, ByVal value As Object) As MethodHandle
			If type.primitive Then
				If type Is GetType(void) Then Throw newIllegalArgumentException("void type")

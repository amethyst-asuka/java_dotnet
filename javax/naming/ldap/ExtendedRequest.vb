...
Namespace javax.naming.ldap
	...
	''' <seealso cref= LdapContext#extendedOperation
	''' @since 1.3 </seealso>
	Public Interface ExtendedRequest
		...
		'''         <tt>ExtendedRequest.requestName</tt> component. </returns>
		...
		'''         component. </returns>
		''' <exception cref="IllegalStateException"> If the encoded value cannot be retrieved
		''' because the request contains insufficient or invalid data/state. </exception>
		...
		''' </summary>
		''' <param name="id">       The possibly null object identifier of the response
		'''                 control. </param>
		...
		''' the response value. It does not include the response OID. </param>
		''' <param name="offset">   The starting position in berValue of the bytes to use. </param>
		''' <param name="length">   The number of bytes in berValue to use.
		''' </param>
		''' <returns> A non-null object. </returns>
		''' <exception cref="NamingException"> if cannot create extended response
		'''     due to an error. </exception>
		''' <seealso cref= ExtendedResponse </seealso>
		Function createExtendedResponse(ByVal id As String, ByVal berValue As SByte(), ByVal offset As Integer, ByVal length As Integer) As ExtendedResponse

...
Namespace javax.sql.rowset
	...
	Public Interface JoinRowSet
		...
		''' </summary>
		''' <param name="rowset"> the <code>RowSet</code> object that is to be added to this
		'''        <code>JoinRowSet</code> object; it must implement the
		'''        <code>Joinable</code> interface and have a match column set </param>
		''' <exception cref="SQLException"> if (1) an empty rowset is added to the to this
		'''         <code>JoinRowSet</code> object, (2) a match column has not been
		'''         set for <i>rowset</i>, or (3) <i>rowset</i>
		'''         violates the active <code>JOIN</code> </exception>
		''' <seealso cref= Joinable#setMatchColumn </seealso>
		Sub addRowSet(ByVal rowset As Joinable)
		...
		'''        <code>Joinable</code> interface </param>
		''' <param name="columnIdx"> an <code>int</code> that identifies the column to become the
		'''         match column </param>
		''' <exception cref="SQLException"> if (1) <i>rowset</i> is an empty rowset or
		'''         (2) <i>rowset</i> violates the active <code>JOIN</code> </exception>
		''' <seealso cref= Joinable#unsetMatchColumn </seealso>
		Sub addRowSet(ByVal rowset As RowSet, ByVal columnIdx As Integer)
		...
		'''        <code>JoinRowSet</code> object; it may implement the
		'''        <code>Joinable</code> interface </param>
		''' <param name="columnName"> the <code>String</code> object giving the name of the
		'''        column to be set as the match column </param>
		''' <exception cref="SQLException"> if (1) <i>rowset</i> is an empty rowset or
		'''         (2) the match column for <i>rowset</i> does not satisfy the
		'''         conditions of the <code>JOIN</code> </exception>
		 Sub addRowSet(ByVal rowset As RowSet, ByVal columnName As String)
		...
		''' does not already have a match column.
		''' </summary>
		''' <param name="rowset"> an array of one or more <code>RowSet</code> objects
		'''        to be added to the <code>JOIN</code>; it may implement the
		'''        <code>Joinable</code> interface </param>
		''' <param name="columnIdx"> an array of <code>int</code> values indicating the index(es)
		'''        of the columns to be set as the match columns for the <code>RowSet</code>
		'''        objects in <i>rowset</i> </param>
		''' <exception cref="SQLException"> if (1) an empty rowset is added to this
		...
		'''        to be added to the <code>JOIN</code>; it may implement the
		'''        <code>Joinable</code> interface </param>
		''' <param name="columnName"> an array of <code>String</code> values indicating the
		'''        names of the columns to be set as the match columns for the
		'''        <code>RowSet</code> objects in <i>rowset</i> </param>
		''' <exception cref="SQLException"> if (1) an empty rowset is added to this
		'''         <code>JoinRowSet</code> object, (2) a match column is not set
		'''         for a <code>RowSet</code> object in <i>rowset</i>, or (3)
		'''         a <code>RowSet</code> object being added violates the active
		...
		''' this union.
		''' </summary>
		''' <returns> a <code>Collection</code> object consisting of the
		'''        <code>RowSet</code> objects added to this <code>JoinRowSet</code>
		'''        object </returns>
		''' <exception cref="SQLException"> if an error occurs generating the
		'''         <code>Collection</code> object to be returned </exception>
'JAVA TO VB CONVERTER TODO TASK: Java wildcard generics are not converted to .NET:
		ReadOnly Property rowSets As ICollection(Of ?)
		...
		'''         object.
		''' </summary>
		''' <returns> a <code>String</code> array of the names of the
		'''         <code>RowSet</code> objects in this <code>JoinRowSet</code>
		'''         object </returns>
		''' <exception cref="SQLException"> if an error occurs retrieving the names of
		'''         the <code>RowSet</code> objects </exception>
		''' <seealso cref= CachedRowSet#setTableName </seealso>
		ReadOnly Property rowSetNames As String()
		...
		''' changes.
		''' </summary>
		''' <returns> a CachedRowSet containing the contents of the JoinRowSet </returns>
		''' <exception cref="SQLException"> if an error occurs assembling the CachedRowSet
		''' object </exception>
		''' <seealso cref= javax.sql.RowSet </seealso>
		''' <seealso cref= javax.sql.rowset.CachedRowSet </seealso>
		''' <seealso cref= javax.sql.rowset.spi.SyncProvider </seealso>
		Function toCachedRowSet() As CachedRowSet

		''' <summary>
		''' Indicates if CROSS_JOIN is supported by a JoinRowSet
		''' implementation
		''' </summary>
		''' <returns> true if the CROSS_JOIN is supported; false otherwise </returns>
		Function supportsCrossJoin() As Boolean

		''' <summary>
		''' Indicates if INNER_JOIN is supported by a JoinRowSet
		''' implementation
		''' </summary>
		''' <returns> true is the INNER_JOIN is supported; false otherwise </returns>
		Function supportsInnerJoin() As Boolean

		''' <summary>
		''' Indicates if LEFT_OUTER_JOIN is supported by a JoinRowSet
		''' implementation
		''' </summary>
		''' <returns> true is the LEFT_OUTER_JOIN is supported; false otherwise </returns>
		Function supportsLeftOuterJoin() As Boolean

		''' <summary>
		''' Indicates if RIGHT_OUTER_JOIN is supported by a JoinRowSet
		''' implementation
		''' </summary>
		''' <returns> true is the RIGHT_OUTER_JOIN is supported; false otherwise </returns>
		Function supportsRightOuterJoin() As Boolean

		''' <summary>
		''' Indicates if FULL_JOIN is supported by a JoinRowSet
		''' implementation
		''' </summary>
		''' <returns> true is the FULL_JOIN is supported; false otherwise </returns>
		Function supportsFullJoin() As Boolean
		...
		''' </summary>
		''' <param name="joinType"> the standard JoinRowSet.XXX static field definition
		''' of a SQL <code>JOIN</code> to re-configure a JoinRowSet instance on
		''' the fly. </param>
		''' <exception cref="SQLException"> if an unsupported <code>JOIN</code> type is set </exception>
		''' <seealso cref= #getJoinType </seealso>
		Property joinType As Integer
		...
		''' WHERE clause used in the JoinRowSet instance </returns>
		''' <exception cref="SQLException"> if an error occurs in generating a representation
		''' of the WHERE clause. </exception>

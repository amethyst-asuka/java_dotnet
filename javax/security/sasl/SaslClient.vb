...
Namespace javax.security.sasl
	...
	''' <seealso cref= SaslClientFactory
	''' 
	''' @author Rosanna Lee
	...
	Public abstract Interface SaslClient
		...
		Function hasInitialResponse() As Boolean
		...
		''' array if the client is to send a response with no data. </returns>
		''' <exception cref="SaslException"> If an error occurred while processing
		''' the challenge or generating a response. </exception>
		Function evaluateChallenge(ByVal challenge As SByte()) As SByte()
		...
		''' {@code IllegalStateException} is thrown.
		''' </summary>
		''' <param name="propName"> The non-null property name. </param>
		...
		''' <exception cref="IllegalStateException"> if this authentication exchange
		''' has not completed </exception>
		...
		Function getNegotiatedProperty(ByVal propName As String) As Object
		 ...
		 ''' the SaslClient instance. This method is idempotent. </summary>

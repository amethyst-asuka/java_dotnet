...
Namespace javax.management.remote.rmi
	...
	Public Interface RMIServer
		...
		''' </returns>
		''' <exception cref="RemoteException"> if there is a communication
		''' exception during the remote method call. </exception>
		...
		''' </summary>
		''' <param name="credentials"> this object specifies the user-defined credentials
		...
		''' </exception>
		''' <exception cref="SecurityException"> if the given credentials do not
		''' allow the server to authenticate the caller successfully. </exception>
		Function newClient(ByVal credentials As Object) As RMIConnection
	End Interface

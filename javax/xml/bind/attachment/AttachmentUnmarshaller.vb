...
Namespace javax.xml.bind.attachment
	...
	''' 
	''' <p>This abstract class identifies if a package requires XOP processing, <seealso cref="#isXOPPackage()"/> and provides retrieval of binary content stored as attachments by content-id.</p>
	''' 
	...
	''' @since JAXB 2.0
	''' </summary>
	''' <seealso cref= javax.xml.bind.Unmarshaller#setAttachmentUnmarshaller(AttachmentUnmarshaller)
	''' </seealso>
	...
	Public MustInherit Class AttachmentUnmarshaller
	   ''' <summary>
	   ''' <p>Lookup MIME content by content-id, <code>cid</code>, and return as a <seealso cref="DataHandler"/>.</p>
	   ''' 
	   ...
	   ''' </summary>
	   ''' <param name="cid"> It is expected to be a valid lexical form of the XML Schema
	   ''' <code>xs:anyURI</code> datatype. If <code><seealso cref="#isXOPPackage()"/>
	   ...
	   ''' 
	   ''' @return
	   '''       a <seealso cref="DataHandler"/> that represents the MIME attachment.
	   ''' </param>
	   ''' <exception cref="IllegalArgumentException"> if the attachment for the given cid is not found. </exception>
	   Public MustOverride Function getAttachmentAsDataHandler(ByVal cid As String) As javax.activation.DataHandler
		...
		''' </returns>
		''' <exception cref="IllegalArgumentException"> if the attachment for the given cid is not found. </exception>
		Public MustOverride Function getAttachmentAsByteArray(ByVal cid As String) As SByte()
		...
		''' </summary>
		''' <returns> true when MIME context is a XOP Document. </returns>

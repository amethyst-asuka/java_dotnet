...
Namespace javax.xml.bind
	...
	''' 
	''' <ul>
	'''   <li><seealso cref="#newInstance(String,ClassLoader) JAXBContext.newInstance( "com.acme.foo:com.acme.bar" )"/> <br/>
	...
	'''   </li>
	'''   <li><seealso cref="#newInstance(Class...) JAXBContext.newInstance( com.acme.foo.Foo.class )"/> <br/>
	...
	'''    these class(es). See <seealso cref="#newInstance(Class...)"/> for details.
	'''   </li>
	''' </ul>
	''' 
	''' <p>
	...
	''' <h3>Unmarshalling</h3>
	''' <p>
	''' The <seealso cref="Unmarshaller"/> class provides the client application the ability
	...
	''' <h3>Marshalling</h3>
	''' <p>
	''' The <seealso cref="Marshaller"/> class provides the client application the ability
	...
	''' <h3>Validation</h3>
	''' <p>
	''' Validation has been changed significantly since JAXB 1.0.  The <seealso cref="Validator"/>
	...
	''' 
	''' In JAXB 2.0, the <seealso cref="Unmarshaller"/> has included convenince methods that expose
	''' the JAXP 1.3 <seealso cref="javax.xml.validation"/> framework.  Please refer to the
	''' <seealso cref="Unmarshaller#setSchema(javax.xml.validation.Schema)"/> API for more
	''' information.
	''' 
	''' 
	...
	''' <ol>
	''' <li>
	''' For each package/class explicitly passed in to the <seealso cref="#newInstance"/> method, in the order they are specified,
	''' <tt>jaxb.properties</tt> file is looked up in its package, by using the associated classloader &mdash;
	''' this is <seealso cref="Class#getClassLoader() the owner class loader"/> for a <seealso cref="Class"/> argument, and for a package
	''' the specified <seealso cref="ClassLoader"/>.
	''' 
	''' <p>
	''' If such a file is discovered, it is <seealso cref="Properties#load(InputStream) loaded"/> as a property file, and
	''' the value of the <seealso cref="#JAXB_CONTEXT_FACTORY"/> key will be assumed to be the provider factory class.
	...
	''' 
	''' <li>
	''' If the system property <seealso cref="#JAXB_CONTEXT_FACTORY"/> exists, then its value is assumed to be the provider
	...
	''' (see <seealso cref="#newInstance(String, ClassLoader, Map)"/> for the parameter semantics.)
	''' or <tt>public static JAXBContext createContet(Class[],Map)</tt> method
	''' (see <seealso cref="#newInstance(Class[], Map)"/> for the parameter semantics) are invoked
	''' to create a <seealso cref="JAXBContext"/>.
	''' 
	...
	''' <seealso cref= S 7.4.1 "Named Packages" in Java Language Specification</a>
	''' @since JAXB1.0 </seealso>
	Public MustInherit Class JAXBContext

		''' <summary>
		''' The name of the property that contains the name of the class capable
		''' of creating new <tt>JAXBContext</tt> objects.
		''' </summary>
		Public Const JAXB_CONTEXT_FACTORY As String = "javax.xml.bind.context.factory"
		...
		''' <seealso cref="#newInstance(String,ClassLoader)"/> method with
		''' the context class loader of the current thread.
		''' </summary>
		''' <exception cref="JAXBException"> if an error was encountered while creating the
		...
		''' </ol> </exception>
		Public Shared Function newInstance(ByVal contextPath As String) As JAXBContext

			'return newInstance( contextPath, JAXBContext.class.getClassLoader() );
			Return newInstance(contextPath, contextClassLoader)
		End Function
		...
		''' are reachable, as defined in <seealso cref="#newInstance(Class...)"/>, from the
		...
		''' </summary>
		''' <param name="contextPath"> list of java package names that contain schema
		'''                    derived class and/or java to schema (JAXB-annotated)
		'''                    mapped classes </param>
		''' <param name="classLoader">
		'''      This class loader will be used to locate the implementation
		'''      classes.
		''' </param>
		''' <returns> a new instance of a <tt>JAXBContext</tt> </returns>
		''' <exception cref="JAXBException"> if an error was encountered while creating the
		'''                       <tt>JAXBContext</tt> such as
		''' <ol>
		'''   <li>failure to locate either ObjectFactory.class or jaxb.index in the packages</li>
		...
		'''   <li>mixing schema derived packages from different providers on the same contextPath</li>
		''' </ol> </exception>
		Public Shared Function newInstance(ByVal contextPath As String, ByVal classLoader As ClassLoader) As JAXBContext

			Return newInstance(contextPath,classLoader,java.util.Collections.emptyMap(Of String, Object)())
		End Function
		...
		''' 
		''' <p>
		''' This is mostly the same as <seealso cref="JAXBContext#newInstance(String, ClassLoader)"/>,
		''' but this version allows you to pass in provider-specific properties to configure
		''' the instantiation of <seealso cref="JAXBContext"/>.
		''' 
		''' <p>
		...
		''' </summary>
		''' <param name="contextPath"> list of java package names that contain schema derived classes </param>
		''' <param name="classLoader">
		'''      This class loader will be used to locate the implementation classes. </param>
		...
		Public Shared Function newInstance(Of T1)(ByVal contextPath As String, ByVal classLoader As ClassLoader, ByVal properties As IDictionary(Of T1)) As JAXBContext
	...
	'    }
		...
		''' 
		''' For example, in the following Java code, if you do
		''' <tt>newInstance(Foo.class)</tt>, the newly created <seealso cref="JAXBContext"/>
		...
		''' The steps involved in discovering the JAXB implementation is discussed in the class javadoc.
		''' </summary>
		''' <param name="classesToBeBound">
		'''      list of java classes to be recognized by the new <seealso cref="JAXBContext"/>.
		'''      Can be empty, in which case a <seealso cref="JAXBContext"/> that only knows about
		'''      spec-defined classes will be returned.
		''' 
		''' @return
		'''      A new instance of a <tt>JAXBContext</tt>. Always non-null valid object.
		...
		''' 
		''' @since JAXB2.0 </exception>
		Public Shared Function newInstance(ParamArray ByVal classesToBeBound As Type()) As JAXBContext

			Return newInstance(classesToBeBound,java.util.Collections.emptyMap(Of String, Object)())
		End Function
		...
		''' 
		''' <p>
		''' An overloading of <seealso cref="JAXBContext#newInstance(Class...)"/>
		''' to configure 'properties' for this instantiation of <seealso cref="JAXBContext"/>.
		''' 
		''' <p>
		''' The interpretation of properties is up to implementations. Implementations should
		''' throw <tt>JAXBException</tt> if it finds properties that it doesn't understand.
		...
		'''      list of java classes to be recognized by the new <seealso cref="JAXBContext"/>.
		'''      Can be empty, in which case a <seealso cref="JAXBContext"/> that only knows about
		'''      spec-defined classes will be returned. </param>
		''' <param name="properties">
		'''      provider-specific properties. Can be null, which means the same thing as passing
		'''      in an empty map.
		''' 
		...
		''' 
		''' @since JAXB2.0 </exception>
		Public Shared Function newInstance(Of T1)(ByVal classesToBeBound As Type(), ByVal properties As IDictionary(Of T1)) As JAXBContext

			If classesToBeBound Is Nothing Then Throw New System.ArgumentException

			' but it is an error to have nulls in it.
			For i As Integer = classesToBeBound.Length - 1 To 0 Step -1
				If classesToBeBound(i) Is Nothing Then Throw New System.ArgumentException
			Next i
		...
		''' </summary>
		''' <returns> an <tt>Unmarshaller</tt> object
		''' </returns>
		''' <exception cref="JAXBException"> if an error was encountered while creating the
		'''                       <tt>Unmarshaller</tt> object </exception>
		Public MustOverride Function createUnmarshaller() As Unmarshaller
		...
		''' </summary>
		''' <returns> a <tt>Marshaller</tt> object
		''' </returns>
		''' <exception cref="JAXBException"> if an error was encountered while creating the
		'''                       <tt>Marshaller</tt> object </exception>
		Public MustOverride Function createMarshaller() As Marshaller


		''' <summary>
		''' <seealso cref="Validator"/> has been made optional and deprecated in JAXB 2.0.  Please
		''' refer to the javadoc for <seealso cref="Validator"/> for more detail.
		''' <p>
		''' Create a <tt>Validator</tt> object that can be used to validate a
		''' java content tree against its source schema.
		''' </summary>
		''' <returns> a <tt>Validator</tt> object
		''' </returns>
		''' <exception cref="JAXBException"> if an error was encountered while creating the
		'''                       <tt>Validator</tt> object </exception>
		''' @deprecated since JAXB2.0 
		Public MustOverride Function createValidator() As Validator

		''' <summary>
		''' Creates a <tt>Binder</tt> object that can be used for
		''' associative/in-place unmarshalling/marshalling.
		''' </summary>
		''' <param name="domType"> select the DOM API to use by passing in its DOM Node class.
		''' </param>
		''' <returns> always a new valid <tt>Binder</tt> object.
		''' </returns>
		...
		''' 
		''' @since JAXB2.0 </exception>
		Public Overridable Function createBinder(Of T)(ByVal domType As Type) As Binder(Of T)
		...
		''' <returns> always a new valid <tt>Binder</tt> object.
		''' 
		''' @since JAXB2.0 </returns>
		Public Overridable Function createBinder() As Binder(Of org.w3c.dom.Node)
			Return createBinder(GetType(org.w3c.dom.Node))
		End Function
		...
		''' 
		''' @since JAXB2.0 </exception>
		Public Overridable Function createJAXBIntrospector() As JAXBIntrospector
			' to make JAXB 1.0 implementations work, this method must not be
			' abstract
			Throw New System.NotSupportedException
		End Function
		...
		''' </summary>
		''' <param name="outputResolver">
		'''      this object controls the output to which schemas
		'''      will be sent.
		''' </param>
		''' <exception cref="IOException">
		'''      if <seealso cref="SchemaOutputResolver"/> throws an <seealso cref="IOException"/>.
		''' </exception>
		''' <exception cref="UnsupportedOperationException">
		'''      Calling this method on JAXB 1.0 implementations will throw
		'''      an UnsupportedOperationException.
		...
		Public Overridable Sub generateSchema(ByVal outputResolver As SchemaOutputResolver)
			' to make JAXB 1.0 implementations work, this method must not be
			' abstract
			Throw New System.NotSupportedException
		End Sub

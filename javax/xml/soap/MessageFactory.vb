...
Namespace javax.xml.soap
	...
	''' </UL>
	''' </summary>
	Public MustInherit Class MessageFactory

		Friend Const DEFAULT_MESSAGE_FACTORY As String = "com.sun.xml.internal.messaging.saaj.soap.ver1_1.SOAPMessageFactory1_1Impl"

		Private Const MESSAGE_FACTORY_PROPERTY As String = "javax.xml.soap.MessageFactory"
		...
		''' 
		''' </summary>
		''' <returns> a new instance of a <code>MessageFactory</code>
		''' </returns>
		''' <exception cref="SOAPException"> if there was an error in creating the
		'''            default implementation of the
		'''            <code>MessageFactory</code>. </exception>
		''' <seealso cref= SAAJMetaFactory </seealso>

		Public Shared Function newInstance() As MessageFactory


			Try
				Dim factory As MessageFactory = CType(FactoryFinder.find(MESSAGE_FACTORY_PROPERTY, DEFAULT_MESSAGE_FACTORY, False), MessageFactory)

				If factory IsNot Nothing Then Return factory
				Return newInstance(SOAPConstants.SOAP_1_1_PROTOCOL)

			Catch ex As Exception
				Throw New SOAPException("Unable to create message factory for SOAP: " & ex.Message)
			End Try
		...
		''' <returns> a new instance of a <code>MessageFactory</code>
		''' </returns>
		''' <param name="protocol">  a string constant representing the class of the
		...
		''' </param>
		''' <exception cref="SOAPException"> if there was an error in creating the
		'''            specified implementation of  <code>MessageFactory</code>. </exception>
		''' <seealso cref= SAAJMetaFactory
		''' @since SAAJ 1.3 </seealso>
		Public Shared Function newInstance(ByVal protocol As String) As MessageFactory
			Return SAAJMetaFactory.instance.newMessageFactory(protocol)
		End Function
		...
		''' </summary>
		''' <returns> a new <code>SOAPMessage</code> object </returns>
		''' <exception cref="SOAPException"> if a SOAP error occurs </exception>
		''' <exception cref="UnsupportedOperationException"> if the protocol of this
		'''      <code>MessageFactory</code> instance is <code>DYNAMIC_SOAP_PROTOCOL</code> </exception>
		Public MustOverride Function createMessage() As SOAPMessage
		...
		''' object.
		''' </summary>
		''' <param name="in"> the <code>InputStream</code> object that contains the data
		'''           for a message </param>
		''' <param name="headers"> the transport-specific headers passed to the
		'''        message in a transport-independent fashion for creation of the
		'''        message </param>
		''' <returns> a new <code>SOAPMessage</code> object containing the data from
		'''         the given <code>InputStream</code> object
		''' </returns>
		''' <exception cref="IOException"> if there is a problem in reading data from
		'''            the input stream
		''' </exception>
		''' <exception cref="SOAPException"> may be thrown if the message is invalid
		''' </exception>
		''' <exception cref="IllegalArgumentException"> if the <code>MessageFactory</code>
		...
		'''      <code>IllegalArgumentException</code> for this reason. </exception>
		Public MustOverride Function createMessage(ByVal headers As MimeHeaders, ByVal [in] As java.io.InputStream) As SOAPMessage
	End Class

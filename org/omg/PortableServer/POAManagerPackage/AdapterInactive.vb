Namespace org.omg.PortableServer.POAManagerPackage


	''' <summary>
	''' org/omg/PortableServer/POAManagerPackage/AdapterInactive.java .
	''' Generated by the IDL-to-Java compiler (portable), version "3.2"
	''' from d:/re/puppet/workspace/8-2-build-windows-amd64-cygwin/jdk8u73/6086/corba/src/share/classes/org/omg/PortableServer/poa.idl
	''' Friday, January 29, 2016 5:40:18 PM PST
	''' </summary>

	Public NotInheritable Class AdapterInactive
		Inherits org.omg.CORBA.UserException

	  Public Sub New()
		MyBase.New(AdapterInactiveHelper.id())
	  End Sub ' ctor


	  Public Sub New(ByVal $reason As String)
		MyBase.New(AdapterInactiveHelper.id() & "  " & $reason)
	  End Sub ' ctor

	End Class ' class AdapterInactive

End Namespace